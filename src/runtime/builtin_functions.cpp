...
void BuiltinFunctions::registerBuiltinFunctions() {
    ...
    functions_["upper"] = upper;
    functions_["lower"] = lower;
...
Value BuiltinFunctions::upper(const std::vector<Value>& args) {
    if (args.empty()) return objToValue(new ObjString(""));
...
Value BuiltinFunctions::lower(const std::vector<Value>& args) {
    if (args.empty()) return objToValue(new ObjString(""));
    std::string s = args[0].toString();
