/*
 * KIO C Core ABI – C++ bridge implementation
 *
...
static kio_value to_c_value(const Value& v) {
    ...
    switch (v.type) {
        case ValueType::VAL_NIL:
            out.type = KIO_VAL_NIL;
            out.as.number = 0.0;
            out.as.boolean = 0;
            out.as.obj = nullptr;
            break;
        case ValueType::VAL_BOOL:
            out.type = KIO_VAL_BOOL;
            out.as.boolean = v.as.boolean ? 1 : 0;
            out.as.number = 0.0;
            out.as.obj = nullptr;
            break;
        case ValueType::VAL_NUMBER:
            out.type = KIO_VAL_NUMBER;
...
static kio_status from_interpret_result(InterpretResult r) {
    switch (r) {
        case InterpretResult::OK:            return KIO_STATUS_OK;
        case InterpretResult::COMPILE_ERROR: return KIO_STATUS_COMPILE_ERROR;
        case InterpretResult::RUNTIME_ERROR: return KIO_STATUS_RUNTIME_ERROR;
    }
    return KIO_STATUS_INTERNAL_ERROR;
}
...
static kio_status check_vec_compatible(const kio_vec_view* a,
    ...
    if (!a || !b || !out) return KIO_STATUS_INVALID_ARGUMENT;
    if (!a->data || !b->data || !out->data) return KIO_STATUS_INVALID_ARGUMENT;
    if (a->stride == 0 || b->stride == 0 || out->stride == 0)
        return KIO_STATUS_INVALID_ARGUMENT;

    // Use the minimum logical length across inputs.
    size_t len = a->length;
    if (b->length < len) len = b->length;
    if (out->length < len) len = out->length;
    return (len == 0) ? KIO_STATUS_INVALID_ARGUMENT : KIO_STATUS_OK;
}
...
int kio_is_number(kio_value v) { return v.type == KIO_VAL_NUMBER; }
int kio_is_bool  (kio_value v) { return v.type == KIO_VAL_BOOL; }
int kio_is_nil   (kio_value v) { return v.type == KIO_VAL_NIL; }
int kio_is_obj   (kio_value v) { return v.type == KIO_VAL_OBJ; }
...
kio_status kio_chunk_write(kio_chunk* chunk, uint8_t byte, int line) {
    if (!chunk) return KIO_STATUS_INVALID_ARGUMENT;
    chunk->chunk.write(byte, line);
    return KIO_STATUS_OK;
}

kio_status kio_chunk_add_constant(kio_chunk* chunk,
                                  kio_value   value,
                                  int*        out_index) {
    if (!chunk || !out_index) return KIO_STATUS_INVALID_ARGUMENT;
...
kio_status kio_vm_execute(kio_vm*          vm,
                          ...
                          kio_error*       out_error) {
    if (!vm || !chunk) return KIO_STATUS_INVALID_ARGUMENT;
...
kio_status kio_vm_register_native(kio_vm*       vm,
    ...
    // native functions can be called from KIO code.
    return KIO_STATUS_INTERNAL_ERROR;
}
...
kio_status kio_vec_dot(const kio_vec_view* a,
                       const kio_vec_view* b,
                       double*             out) {
    if (!out) return KIO_STATUS_INVALID_ARGUMENT;
    if (!a || !b || !a->data || !b->data) return KIO_STATUS_INVALID_ARGUMENT;
    if (a->stride == 0 || b->stride == 0) return KIO_STATUS_INVALID_ARGUMENT;

    size_t len = a->length;
    if (b->length < len) len = b->length;
    if (len == 0) return KIO_STATUS_INVALID_ARGUMENT;
...
kio_status kio_vec_normalize(const kio_vec_view* a,
                             kio_vec_view*       out) {
    if (!a || !out || !a->data || !out->data) return KIO_STATUS_INVALID_ARGUMENT;
    if (a->stride == 0 || out->stride == 0) return KIO_STATUS_INVALID_ARGUMENT;
    ...
    if (mag_sq == 0.0) return KIO_STATUS_RUNTIME_ERROR;
